...
/** 루트 Y 오프셋 — 프레임을 살짝 아래로 내려 하단 여백·천장 여유 균형 */
const SCENE_ROOT_Y = -0.16;
...
const TARGET_DRAG_SCALE = (VINYL_RECORD_MESH_R * TT_GROUP_SCALE) / DRAG_VINYL_MESH_R;
/** 드래그 스케일 lerp — z가 이쪽(덱)일수록 TARGET에 가깝게 */
const DRAG_Z_SHELF = -3.28;
const DRAG_Z_DECK = -1.32;
const DRAG_SCALE_AT_SHELF = 0.84;
...
const DraggingLP = React.memo(function DraggingLP({
  ...
  useFrame((_, delta) => {
    ...
    const depthT = THREE.MathUtils.smoothstep(z, DRAG_Z_SHELF, DRAG_Z_DECK);
    ...
    const tiltTarget = isOverTurntable ? 0 : 0.44;
    carryTilt.current += (tiltTarget - carryTilt.current) * Math.min(1, delta * 8);
...
function CameraBreathing() {
  ...
  useEffect(() => {
    // 덱 쪽으로 줌인 — 드래그 LP 원근 점프 완화·플레이스먼트 정밀도
    camera.position.set(0, 1.92, 3.95);
    camera.lookAt(0, 0.88, -0.48);
  }, []);
...
export function VinylShopScene({
  ...
  return (
    <View style={styles.root}>
      <Canvas
        style={styles.canvas}
        camera={{ position: [0, 1.92, 3.95], fov: 57.5, near: 0.1, far: 25 }}
        dpr={[1, 1.5]}
        performance={{ min: 0.5 }}
        gl={{
          antialias: false,
